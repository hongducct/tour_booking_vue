...
// Form inputs
const cityCode = ref('DAD')
const checkInDate = ref('2025-06-09')
const checkOutDate = ref('2025-06-10')
const adults = ref(2)
const rooms = ref(1)
...
// Payment methods (restored original options)
...
const searchHotels = async () => {
  ...
  try {
    ...
    if (data.success) {
      hotels.value = data.data.map((item) => ({
        ...
        offers: item.offers.map((offer) => ({
          ...
          acceptedCreditCards: offer.policies.prepay?.acceptedPayments.creditCards,
        })),
...
/**
 * Handle hotel and offer selection
 */
const selectHotel = (hotel, offer) => {
  ...
  bookingForm.value.roomType = offer.room.category || 'DELUXE_ROOM'

  Object.keys(bookingForm.value).forEach((key) => {
    if (typeof bookingForm.value[key] === 'string' && key !== 'roomType' && key !== 'paymentMethod') {
...
const processBooking = async () => {
  ...
  try {
    ...
    if (bookingForm.value.paymentMethod === 'credit_card') {
      ...
      if (
        selectedOffer.value.acceptedCreditCards &&
        !selectedOffer.value.acceptedCreditCards.includes(bookingForm.value.cardType)
      ) {
        throw new Error('Thẻ tín dụng không được chấp nhận cho ưu đãi này.')
      }
    }
 ...
 */
const formatCancellationDeadline = (deadline) => {
  if (!deadline) return 'Không hoàn tiền'
  const date = new Date(deadline)
  return `Hủy miễn phí trước ${date.toLocaleString('vi-VN', {
    day: '2-digit',
    month: '2-digit',
    year: 'numeric',
    hour: '2-digit',
...
<template>
  <div class="w-full max-w-6xl mx-auto px-4 mt-8">
    <div
      class="backdrop-blur-sm shadow-xl/30 bg-white/80 rounded-3xl border border-white/20 p-8 mb-12"
    >
          ...
          <div class="space-y-2">
            ...
            <div class="relative group">
              <input
                ...
                type="date"
                :min="today"
                class="w-full p-4 pr-12 text-lg border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 bg-white/90 backdrop-blur-sm hover:border-blue-300 date-input"
                required
              />
              <CalendarDaysIcon
    ...
    <div v-if="isLoading" class="text-center py-16">
      ...
      <p class="text-gray-600 text-lg">Chúng tôi đang tìm những khách sạn tuyệt vời nhất cho bạn</p>
    </div>
            ...
            <div class="lg:w-3/5 p-8">
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center mb-4">
                    <div class="flex space-x-1">
                      <StarIcon
                        v-for="(filled, index) in getStarRating(4)"
                        :key="index"
                        :class="[
                    ...
                    <span class="ml-3 text-gray-600 font-medium">4 sao</span>
                  </div>

                  <div class="flex items-center text-gray-600 mb-6">
                    <MapPinIcon class="h-5 w-5 mr-2 text-blue-500" />
                    <span class="font-medium">{{ vietnamCities.find(c => c.code === hotel.cityCode)?.name || 'Trung tâm thành phố' }}</span>
                  </div>

                  <div class="flex flex-wrap gap-4 text-sm">
                    <div class="flex items-center bg-blue-50 text-blue-700 px-3 py-2 rounded-full">
              ...
              <div class="space-y-4">
                <div
                  ...
                  class="border-t pt-4"
                >
                  <p class="text-gray-600 font-medium">{{ offer.room.description }}</p>
                  <p class="text-sm text-gray-500">Bữa ăn: {{ offer.boardType }}</p>
                  <p class="text-sm text-gray-500">{{ formatCancellationDeadline(offer.cancellationDeadline) }}</p>
                  <div class="flex justify-between items-center mt-2">
                    ...
                    <button
                      @click="selectHotel(hotel, offer)"
                      class="group/btn bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-2 rounded-2xl hover:from-blue-700 hover:to-purple-700 transition-all duration-300 font-bold text-sm shadow-lg hover:shadow-xl transform hover:scale-105"
                    >
                      Đặt ngay
                    </button>
                  </div>
    ...
    <div class="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <div v-if="bookingSuccess" class="p-6 text-center">
        ...
        <div v-if="bookingResult" class="space-y-2 text-gray-600">
          <p><strong>Mã đặt phòng:</strong> {{ bookingResult.bookingReference }}</p>
      ...
      <form v-else @submit.prevent="processBooking" class="p-6 space-y-6">
        <div class="bg-green-50 rounded-lg p-4 mb-6">
          ...
          <div v-if="selectedHotel && selectedOffer" class="text-sm text-green-700">
            <p><strong>Khách sạn:</strong> {{ selectedHotel.name }}</p>
            <p><strong>Phòng:</strong> {{ selectedOffer.room.description }}</p>
            <p><strong>Bữa ăn:</strong> {{ selectedOffer.boardType }}</p>
            <p>
              <strong>Ngày:</strong> {{ formatDate(checkInDate) }} - {{ formatDate(checkOutDate) }}
            </p>
            <p><strong>Số đêm:</strong> {{ numberOfNights }} đêm</p>
            <p>
              <strong>Giá:</strong>
              {{ formatPrice(selectedOffer.price.total * numberOfNights, selectedOffer.price.currency) }}
            </p>
            <p><strong>Chính sách hủy:</strong> {{ formatCancellationDeadline(selectedOffer.cancellationDeadline) }}</p>
          </div>
        </div>
